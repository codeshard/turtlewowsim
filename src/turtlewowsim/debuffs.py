...
from turtlewowsim.classes import Ignite, ImprovedShadowBolt
...
class Debuffs:
    ...
    def is_dot_active(self, dot_class, owner):
        ...
        if dot_class not in self.dots:
            return False
        return (
            owner in self.dots[dot_class]
            and self.dots[dot_class][owner].is_active()
        )
    ...
    def get_dot_time_left(self, dot_class, owner):
        ...
        # Total time = time until next tick + time for remaining ticks after
        # that
