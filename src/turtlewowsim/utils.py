...
class DamageMeter:
    ...
    def register_spell_dmg(
        ...
        final_dmg = self._register(
            char_name, spell_name, dmg, cast_time, aoe, increment_cast
        )
        self.total_spell_dmg += final_dmg

    def register_proc_dmg(
        self, char_name: str, spell_name: str, dmg: int, aoe=False
    ):
        final_dmg = self._register(char_name, spell_name, dmg, 0, aoe)
        self.total_proc_dmg += final_dmg

    def register_dot_cast(
        self, char_name: str, spell_name: str, cast_time: float
    ):
        self._register(
            char_name, spell_name, 0, cast_time, aoe=False, increment_cast=True
        )

    def register_dot_dmg(
        self, char_name: str, spell_name: str, dmg: int, aoe=False, cast_time=0
    ):
        final_dmg = self._register(
            char_name,
            spell_name,
    ...
    def register_ignite_dmg(
        self, char_name: str, dmg: int, aoe=False, increment_cast=False
    ):
        final_dmg = self._register(
            char_name, "ignite", dmg, 0, aoe, increment_cast=increment_cast
        )
        self.total_ignite_dmg += final_dmg

    def get_total_dmg(self):
        return (
            self.total_spell_dmg + self.total_dot_dmg + self.total_ignite_dmg
        )
    ...
    def raid_dmg(self):
        ...
        return round(
            total_raid_dmg / total_time / len(self.character_dmg.keys()), 1
        )
    ...
    def report(self):
        ...
        for character in self.env.characters:
            casts[character.name] = sum(
                self.character_num_casts[character.name].values()
            )

        for name, dps in self.dps().items():
            print(
                f"{name.ljust(JUSTIFY, ' ')}: {dps} dps in {casts[name]} casts"
            )
    ...
    def detailed_report(self):
        ...
        for char_name in self.character_dmg.keys():
            ...
            for spell_name, num_casts in self.character_num_casts[
                char_name
            ].items():
                total_dmg = 0
                percent_dmg = 0
                ...
                if (
                    char_name in self.character_spell_dmg
                    and spell_name in self.character_spell_dmg[char_name]
                ):
                    total_dmg = round(
                        self.character_spell_dmg[char_name][spell_name]
                    )
                    percent_dmg = round(100 * total_dmg / total_char_dmg, 1)
                    avg_dmg = mean([total_dmg / num_casts])

                if (
                    char_name in self.character_spell_cast_time
                    and spell_name in self.character_spell_cast_time[char_name]
                ):
                    avg_cast_time = mean(
                        [
                            self.character_spell_cast_time[char_name][
                ...
                if (
                    char_name in self.character_num_ticks
                    and spell_name in self.character_num_ticks[char_name]
                ):
    ...
    def per_spell_data(self, char_name):
        ...
        for spell_name, num_casts in self.character_num_casts[
            ...
            if (
                ...
                avg_cast_time = mean(
                    [
                        self.character_spell_cast_time[char_name][spell_name]
                        / num_casts
                    ]
                )
                avg_dps = _round(avg_dmg / avg_cast_time)
