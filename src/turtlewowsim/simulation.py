...
from turtlewowsim.utils import mean, mean_percentage
    ...
    try:
        ...
        for i in chunk_range:
            ...
            for character, mdps in dps_results.items():
                ...
                chunk_results["casts"][character].append(
                    env.meter.total_casts(character)
                )

            for character in characters:
                char_name = character.name
                chunk_results["partial_resists"][char_name].append(
                    character.num_partials
                )
                chunk_results["resists"][char_name].append(
                    character.num_resists
                )

                if char_name not in chunk_results["per_spell_data"]:
                    chunk_results["per_spell_data"][char_name] = (
                        env.meter.per_spell_data(char_name)
                    )
                else:
                    for spell_name, data in env.meter.per_spell_data(
                        char_name
                    ).items():
                        if (
                            spell_name
                            in chunk_results["per_spell_data"][char_name]
                        ):
                ...
                if char_name not in chunk_results["buff_uptime"]:
                    chunk_results["buff_uptime"][char_name] = (
                        character.buff_uptimes
                    )
                else:
                    for (
                        buff_name,
                        buff_uptime,
                    ) in character.buff_uptimes.items():
                        if (
                            buff_name
                            in chunk_results["buff_uptime"][char_name]
                        ):
                            chunk_results["buff_uptime"][char_name][
                                buff_name
                            ] += buff_uptime
            ...
            for (
                debuff_name,
                debuff_uptime,
            ) in env.debuffs.debuff_uptimes.items():
            ...
            chunk_results["total_ignite_dmg"].append(
                env.meter.total_ignite_dmg
            )
            ...
            chunk_results[">=2 stack uptime"].append(
                ignite.uptime_gte_2_stacks
            )
            chunk_results[">=3 stack uptime"].append(
                ignite.uptime_gte_3_stacks
            )
            chunk_results[">=4 stack uptime"].append(
                ignite.uptime_gte_4_stacks
            )
            ...
            chunk_results["Total added dot dmg"].append(
                isb.total_added_dot_dmg
            )
            chunk_results["Total added spell dmg"].append(
                isb.total_added_spell_dmg
            )
...
class Simulation:
        ...
        if use_multiprocessing:
            ...
            with ProcessPoolExecutor(
                ...
                try:
                    ...
                    futures = [
                        executor.submit(run_simulation, args, chunk)
                        for chunk in chunks
                    ]
                    ...
                    for idx, future in enumerate(futures):
                        try:
                            ...
                            for f in futures[idx + 1 :]:
                                f.cancel()
                            raise RuntimeError(
                                f"Chunk {idx} timed out after 10 seconds"
                            )
                        except Exception as e:
            ...
            if error_chunks:
                error_msg = "\n".join(
                    f"Chunk {ec['chunk']}: {ec['error']}"
                    for ec in error_chunks[:3]
                )
            ...
            for i in trange(iterations, ascii=True):
                ...
                for character, mdps in dps_results.items():
                    ...
                    self.results["casts"][character].append(
                        env.meter.total_casts(character)
                    )

                for character in self.characters:
                    char_name = character.name
                    self.results["partial_resists"][char_name].append(
                        character.num_partials
                    )
                    self.results["resists"][char_name].append(
                        character.num_resists
                    )

                    if char_name not in self.results["per_spell_data"]:
                        self.results["per_spell_data"][char_name] = (
                            env.meter.per_spell_data(char_name)
                        )
                    else:
                        for spell_name, data in env.meter.per_spell_data(
                            ...
                            if (
                                spell_name
                                in self.results["per_spell_data"][char_name]
                            ):
                                for key, value in data.items():
                                    self.results["per_spell_data"][char_name][
                                        spell_name
                    ...
                    if char_name not in self.results["buff_uptime"]:
                        self.results["buff_uptime"][char_name] = (
                            character.buff_uptimes
                        )
                    else:
                        for (
                            ...
                            if (
                                buff_name
                                in self.results["buff_uptime"][char_name]
                            ):
                                self.results["buff_uptime"][char_name][
                                    buff_name
                                ] += buff_uptime
                ...
                self.results["total_ignite_dmg"][i] = (
                    env.meter.total_ignite_dmg
                )
                ...
                self.results[">=2 stack uptime"][i] = (
                    ignite.uptime_gte_2_stacks
                )
                self.results[">=3 stack uptime"][i] = (
                    ignite.uptime_gte_3_stacks
                )
                self.results[">=4 stack uptime"][i] = (
                    ignite.uptime_gte_4_stacks
                )
                ...
                self.results["Total added dot dmg"][i] = (
                    isb.total_added_dot_dmg
                )
                self.results["Total added spell dmg"][i] = (
                    isb.total_added_spell_dmg
                )
    ...
    def _merge_results(self, results):
        ...
        for chunk in results:
            ...
            for char in chunk["dps"]:
                ...
                merged["partial_resists"][char].extend(
                    chunk["partial_resists"][char]
                )
                ...
                if char in chunk["per_spell_data"]:
                    for spell, data in chunk["per_spell_data"][char].items():
                        for metric, value in data.items():
                            merged["per_spell_data"][char][spell][metric] += (
                                value
                            )
    ...
    def report(self, verbosity=1):
        ...
        if verbosity > 1:
            ...
            for char in self.results["dps"]:
                ...
                msg = (
                    f"{self._justify(label)}: {mean_dps} in {mean_casts} casts"
                )
        ...
        if verbosity > 2:
            print("------ Per Spell Data ------")
            chars_sorted = sorted(
                self.results["dps"].keys(),
                key=lambda x: mean(self.results["dps"][x]),
                reverse=True,
            )
            for char in chars_sorted:
                iterations = self.results["iterations"]
                print(f"{char}:")
                for spell_name, data in self.results["per_spell_data"][
                    char
                ].items():
                    ...
                    avg_cast_time = round(
                        data["avg_cast_time"] / iterations, 2
                    )
        ...
        if verbosity > 2:
            ...
            for char in chars_sorted:
                iterations = self.results["iterations"]
                print(f"{char}:")
                for buff_name, total_uptime in self.results["buff_uptime"][
                    char
                ].items():
            ...
            print("------ Debuff Uptime ------")
            for debuff_name, total_uptime in self.results[
                "debuff_uptime"
            ].items():
                avg_uptime = round(
                    total_uptime / self.results["iterations"], 1
                )
    ...
    def histogram_report_individual(self):
        for char in self.results["dps"]:
            fig = px.histogram(
                x=self.results["dps"][char], histnorm="probability density"
            )
    ...
    def histogram_report_overlay(self):
        ...
        fig.update_layout(title="DPS Distributions", barmode="overlay")
