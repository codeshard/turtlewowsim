...
class Warlock(Character):
    ...
    def _spell(
        ...
        if hit and spell == Spell.CORRUPTION:
            ...
            self.env.debuffs.add_dot(
                CorruptionDot, self, max(base_cast_time, self.env.GCD)
            )
    ...
    def _shadow_dot(
        ...
        if not hit:
            ...
            if spell == Spell.CURSE_OF_SHADOW:
                ...
                self.env.debuffs.add_dot(
                    CorruptionDot, self, max(casting_time, self.env.GCD)
                )
    ...
    def _dark_harvest_tick(
        self, tick_time: float = 3, dmg_multiplier: float = None
    ):
        dmg = 158
    ...
    def _immo_conflag_soulfire_searing(
        ...
        while True:
            ...
            if (
                ...
                and self.env.debuffs.is_dot_active(ImmolateDot, self)
            ):
    ...
    # affliction
    @simrotation(
        "(Affli) CoA -> Corruption -> Siphon Life -> Harvest -> Drain Soul"
    )
