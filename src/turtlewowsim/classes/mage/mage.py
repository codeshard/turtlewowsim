...
class Mage(Character):
    def __init__(
        ...
        if self.tal:
            ...
            if self.tal.ice_shards > 0:
                self.damage_type_crit_mult[DamageType.FROST] += (
                    self.tal.ice_shards * 0.1
                )

            if self.tal.arcane_potency:
                self.damage_type_crit_mult[DamageType.ARCANE] += (
                    self.tal.arcane_potency * 0.25
                )
    ...
    def _setup_cds(self):
        self.fire_blast_cd = FireBlastCooldown(
            self, self.tal.fire_blast_cooldown
        )
        self.frost_nova_cd = FrostNovaCooldown(
            self, self.tal.frost_nova_cooldown
        )
        self.cold_snap_cd = ColdSnapCooldown(self)
        self.cone_of_cold_cd = ConeOfColdCooldown(self)
        self.icicles_cd = IciclesCooldown(self)
        self.arcane_rupture_cd = ArcaneRuptureCooldown(
            self, self.tal.accelerated_arcana
        )
        self.arcane_surge_cd = ArcaneSurgeCooldown(
            self, self.tal.accelerated_arcana
        )
    ...
    def attach_env(self, env: Environment):
        ...
        if self.opts.start_with_ice_barrier:
            self._ice_barrier_expiration = (
                self.opts.starting_ice_barrier_duration
            )

    def _ice_barrier_active(self):
        return self._ice_barrier_expiration >= self.env.now

    def _arcane_rupture_missiles(
        ...
        while True:
            ...
            if self.arcane_rupture_cd.usable:
                ...
                if (
                    self.opts.use_presence_of_mind_on_cd
                    and self.cds.presence_of_mind.usable
                ):
    ...
    def _arcane_surge_rupture_missiles(
        self, cds: CooldownUsages = CooldownUsages(), delay=2
    ):
        self._use_cds(cds)
        yield from self._random_delay(delay)
        ...
        while True:
            ...
            if (
                self.arcane_surge_cd.usable
                and not self.has_trinket_or_cooldown_haste()
            ):
    ...
    def _arcane_surge_fireblast_rupture_missiles(
        self, cds: CooldownUsages = CooldownUsages(), delay=2
    ):
        self._use_cds(cds)
        yield from self._random_delay(delay)
    ...
    def _arcane_rupture_surge_missiles(
        self, cds: CooldownUsages = CooldownUsages(), delay=2
    ):
        self._use_cds(cds)
        yield from self._random_delay(delay)
    ...
    def _icicle_frostbolts(
        ...
        while True:
            ...
            if self.tal.ice_barrier and not self._ice_barrier_active():
                ...
                self.opts.use_frostnova_for_icicles
                and self.frost_nova_cd.usable
            ):
                yield from self._frost_nova()
            elif (
                self.opts.use_frostnova_for_icicles
                and self.opts.use_cold_snap_for_nova
                and self.cold_snap_cd.usable
            ):
                self.cold_snap_cd.activate()
                yield from self._frost_nova()
            elif (
                self.opts.use_icicles_without_flash_freeze
                and self.icicles_cd.usable
            ):
                yield from self._icicles_channel()
            else:
                yield from self._frostbolt()

    def _icicle_coc_frostbolts(
        self, cds: CooldownUsages = CooldownUsages(), delay=2
    ):
        self._use_cds(cds)
        yield from self._random_delay(delay)
    ...
    def _one_scorch_then_fireballs(
        self, cds: CooldownUsages = CooldownUsages(), delay=2
    ):
    ...
    def _smart_scorch_and_fireblast(
        self, cds: CooldownUsages = CooldownUsages(), delay=2
    ):
    ...
    def _smart_scorch_and_fireblast_and_surge(
        self, cds: CooldownUsages = CooldownUsages(), delay=2
    ):
        """Same as above except fireblast on cd"""
        self._use_cds(cds)
    ...
    def _one_scorch_one_pyro_then_fb(
        self, cds: CooldownUsages = CooldownUsages(), delay=1
    ):
        self._use_cds(cds)
        yield from self._random_delay(delay)
    ...
    def _one_scorch_one_frostbolt_then_fb(
        self, cds: CooldownUsages = CooldownUsages(), delay=1
    ):
        self._use_cds(cds)
        yield from self._random_delay(delay)
    ...
    def modify_dmg(self, dmg: int, damage_type: DamageType, is_periodic: bool):
        ...
        if damage_type == DamageType.FIRE and self.tal.fire_power:
            dmg *= (
                ...
                else 1 + 0.02 * self.tal.fire_power
            )
        ...
        if (
            self.tal.ice_barrier
            and damage_type == DamageType.FROST
            and self._ice_barrier_active()
        ):
            dmg *= 1.15
    ...
    def extend_ignite(self):
        ...
        if (
            ignite_time_remaining
            <= self.opts.remaining_seconds_for_ignite_extend
        ):
            if (
                self.opts.extend_ignite_with_fire_blast
                and self.fire_blast_cd.usable
            ):
                yield from self._fire_blast()
                return True

            scorch_cast_time = (
                self._get_cast_time(1.5, DamageType.FIRE) + self.lag
            )
            if (
                self.opts.extend_ignite_with_scorch
                and ignite_time_remaining > scorch_cast_time
            ):
                yield from self._scorch()
                return True
    ...
    def _spell(
        ...
        hit = (
            self._roll_hit(self._get_hit_chance(spell), damage_type)
            if spell != Spell.ARCANE_SURGE
            else True
        )  # arcane surge always hits
        crit = False
        dmg = 0
        ...
        if hit:
            ...
            if (
                self.tal.arcane_instability
                and damage_type == DamageType.ARCANE
            ):
                hit_chance = {1: 8, 2: 16, 3: 25}.get(
                    self.tal.arcane_instability, 0
                )
            ...
            if (
                self.arcane_rupture_cd.is_active()
                and spell == Spell.ARCANE_MISSILE
            ):
                dmg *= 1.25
                arcane_rupture_applied = True

            if self.opts.t35_3_set and spell in {
                Spell.FLAMESTRIKE,
                Spell.CONE_OF_COLD,
                Spell.FROST_NOVA,
                Spell.BLASTWAVE,
    ...
    def _arcane_spell(
        ...
        if spell == Spell.ARCANE_MISSILE and self.tal.temporal_convergence:
            if self.temporal_convergence_cd.usable:
                temporal_convergence_hit = self._roll_proc(
                    5 * self.tal.temporal_convergence
                )
    ...
    def _arcane_missiles_channel(
        self,
        channel_time: float = 5,
        ...
        if self.tal.accelerated_arcana:
            channel_time = channel_time * 0.95
            channel_time /= self.get_haste_factor_for_damage_type(
                DamageType.ARCANE
            )
        ...
        had_sulfuras_proc = (
            self.equipped_items.true_band_of_sulfuras
        ...
        for i in range(num_missiles):
            ...
            if (
                ...
                if not self.arcane_rupture_cd.is_active():
                    yield self.env.timeout(
                        self.opts.delay_when_interrupting_missiles
                    )
                    yield from self._arcane_surge()
                    return
                ...
                if self.arcane_surge_cd.time_left() < time_between_missiles:
                    ...
                    return
            if (
                interrupt_for_rupture
                and self.arcane_rupture_cd.usable
            ...
            if i == 0:
                yield from self._arcane_missile(
                    casting_time=time_between_missiles + self.lag
                )  # initial delay
            else:
                yield from self._arcane_missile(
                    casting_time=time_between_missiles
                )
                if (
    ...
    def _fire_spell(
        ...
        has_bad_ignite = (
            has_5_stack_ignite and self.env.debuffs.ignite.is_suboptimal()
        )
        ...
        if (
            self.hot_streak
            and self.hot_streak.get_stacks() == 5
            and self.opts.pyro_on_max_hot_streak
        ):
        ...
        if crit:
            if self.tal.ignite:
                self.env.debuffs.ignite.refresh(
                    self, dmg, spell, partial_amount < 1, self.tal.ignite
                )

            # check for hot streak
            if self.hot_streak and (
    ...
    def _scorch(self):
        ...
        crit_modifier += (
            self.tal.incinerate_crit
        )  # incinerate added crit (2 or 4%)
    ...
    def _fireball(self):
        min_dmg = 596
        max_dmg = 760
        casting_time = 3.5 - self.tal.improved_fireball * 0.1
        crit_modifier = 0
    ...
    def _frost_spell(
        ...
        crit_modifier += (
            self.env.debuffs.wc_stacks * 2
        )  # winters chill added crit (2% per stack)
        ...
        if hit:
            if self.tal.winters_chill and spell != Spell.FROST_NOVA:
                # roll for whether debuff hits
                winters_chill_hit = self._roll_hit(
                    self._get_hit_chance(spell), DamageType.FROST
                )
                if winters_chill_hit:
                    winters_chill_proc = self.tal.winters_chill * 20
                    if winters_chill_proc == 100 or self._roll_proc(
                        winters_chill_proc
                    ):
            ...
            if self.tal.flash_freeze:
                ...
                if (
                    ...
                    or spell == Spell.FROSTBOLTRK4
                    or spell == Spell.FROSTBOLTRK3
                    or spell == Spell.CONE_OF_COLD
                ):
                    flash_freeze_hit = self._roll_hit(
                        5 * self.tal.frostbite, DamageType.FROST
                    )
                    if flash_freeze_hit and self.tal.flash_freeze < 2:
                        flash_freeze_hit = self._roll_proc(
                            50 * self.tal.flash_freeze
                        )
    ...
    def _icicles_channel(self, channel_time: float = 5):
        ...
        for i in range(num_icicles):
            if i == 0:
                yield from self._icicle(
                    casting_time=time_between_icicles + self.lag
                )  # initial delay
            else:
                yield from self._icicle(casting_time=time_between_icicles)

    @simrotation("(Arcane) Arcane Surge -> Arcane Rupture -> Arcane Missiles")
    def arcane_surge_rupture_missiles(
        self, cds: CooldownUsages = CooldownUsages(), delay=2
    ):
        return partial(
            self._set_rotation, name="arcane_surge_rupture_missiles"
        )(cds=cds, delay=delay)

    @simrotation("(Fire) Smart Scorch -> Fireblast -> Fireball")
    def smart_scorch_and_fireblast(
        self, cds: CooldownUsages = CooldownUsages(), delay=2
    ):
    ...
    def icicle_frostbolts(
        self, cds: CooldownUsages = CooldownUsages(), delay=2
    ):
        return partial(self._set_rotation, name="icicle_frostbolts")(
            cds=cds, delay=delay
        )

    @simrotation("(Frost) Icicle -> Cone of Cold -> Frostbolt (Spam)")
    def icicle_coc_frostbolts(
        self, cds: CooldownUsages = CooldownUsages(), delay=2
    ):
        return partial(self._set_rotation, name="icicle_coc_frostbolts")(
            cds=cds, delay=delay
        )

    @simrotation("(Arcane) Arcane Rupture -> Arcane Missiles")
    def arcane_rupture_missiles(
        self, cds: CooldownUsages = CooldownUsages(), delay=2
    ):
        return partial(self._set_rotation, name="arcane_rupture_missiles")(
            cds=cds, delay=delay
        )
    ...
    def arcane_surge_fireblast_rupture_missiles(
        ...
        return partial(
            self._set_rotation, name="arcane_surge_fireblast_rupture_missiles"
        )(cds=cds, delay=delay)

    @simrotation("(Arcane) Arcane Rupture -> Arcane Surge -> Arcane Missiles")
    def arcane_rupture_surge_missiles(
        self, cds: CooldownUsages = CooldownUsages(), delay=2
    ):
        return partial(
            self._set_rotation, name="arcane_rupture_surge_missiles"
        )(cds=cds, delay=delay)

    @simrotation("(Arcane) Arcane Missiles")
    def arcane_missiles(self, cds: CooldownUsages = CooldownUsages(), delay=2):
        return partial(self._set_rotation, name="arcane_missiles")(
            cds=cds, delay=delay
        )

    @simrotation("(Arcane) Arcane Explosion (Spam)")
    def spam_arcane_explosion(
        self, cds: CooldownUsages = CooldownUsages(), delay=2
    ):
        return partial(self._set_rotation, name="spam_arcane_explosion")(
            cds=cds, delay=delay
        )
    ...
    def spam_pyroblast(self, cds: CooldownUsages = CooldownUsages(), delay=2):
        return partial(self._set_rotation, name="spam_pyroblast")(
            cds=cds, delay=delay
        )
    ...
    @simrotation(
        "(Fire) Smart Scorch -> Fireblast -> Arcane Surge -> Fireball"
    )
    def smart_scorch_and_fireblast_and_surge(
        self, cds: CooldownUsages = CooldownUsages(), delay=2
    ):
        return partial(
            self._set_rotation, name="smart_scorch_and_fireblast_and_surge"
        )(cds=cds, delay=delay)
