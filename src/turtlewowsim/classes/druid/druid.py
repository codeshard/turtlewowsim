...
class Druid(Character):
    ...
    def modify_dmg(self, dmg: int, damage_type: DamageType, is_periodic: bool):
        ...
        if (
            self.nature_eclipse.is_active()
            and damage_type == DamageType.NATURE
        ):
            dmg *= 1.3
        elif (
            self.arcane_eclipse.is_active()
    ...
    def _arcane_spell(
        ...
        hit, crit, dmg, cooldown, partial_amount = yield from self._spell(
            spell=spell,
    ...
    def _starfire(self):
        ...
        if self.balance_of_all_things_stacks > 0:
            casting_time -= 1 if self.opts.set_bonus_3_5_boat else 0.75
            if self.opts.ebb_and_flow_idol:
                casting_time -= 0.2

            self.balance_of_all_things_stacks -= 1

        yield from self._nature_spell(
            spell=Spell.STARFIRE,
            min_dmg=min_dmg,
            max_dmg=max_dmg,
    ...
    def _nature_dot(
        ...
        if not hit:
            ...
            if spell == Spell.INSECT_SWARM:
                self.env.debuffs.add_dot(
                    InsectSwarmDot, self, round(casting_time + cooldown, 2)
                )
    ...
    def insect_swarm_wrath_subrotation(
        self, cds: CooldownUsages = CooldownUsages(), delay=2
    ):
        if (
            not self.env.debuffs.is_dot_active(InsectSwarmDot, self)
            and self.env.remaining_time() > 20
        ):
            yield from self._insect_swarm()
        else:
            yield from self._wrath()

    def insect_swarm_moonfire_wrath_subrotation(
        self, cds: CooldownUsages = CooldownUsages(), delay=2
    ):
        if (
            not self.env.debuffs.is_dot_active(InsectSwarmDot, self)
            ...
            not self.env.debuffs.is_dot_active(MoonfireDot, self)
            and self.env.remaining_time() > 20
        ):
    ...
    def moonfire_starfire_subrotation(
        self, cds: CooldownUsages = CooldownUsages(), delay=2
    ):
        if (
            not self.env.debuffs.is_dot_active(MoonfireDot, self)
    ...
    def moonfire_insect_swarm_starfire_subrotation(
        self, cds: CooldownUsages = CooldownUsages(), delay=2
    ):
        if (
            not self.env.debuffs.is_dot_active(MoonfireDot, self)
    ...
    def _base_rotation(
        ...
        while True:
            self._use_cds(cds)
            if (
                self.opts.starfire_on_balance_of_all_things_proc
                and self.balance_of_all_things_stacks > 0
                and self.arcane_eclipse.is_active()
            ):
                yield from self._starfire()
            elif (
                self.opts.starfire_on_balance_of_all_things_proc
                and self.balance_of_all_things_stacks > 2
            ):
                yield from self._starfire()
            elif (
                self.nature_eclipse.is_active()
                and self.nature_eclipse_rotation
                and not self.opts.ignore_nature_eclipse
            ):
                yield from self.nature_eclipse_rotation(self)
            elif (
                self.arcane_eclipse.is_active()
                and self.arcane_eclipse_rotation
                and not self.opts.ignore_arcane_eclipse
            ):
    ...
    def _spam_starfire(self, cds: CooldownUsages = CooldownUsages(), delay=2):
        ...
        return self._base_rotation(
            cds=cds, delay=delay, rotation_callback=_rotation_callback
        )

    def _insect_swarm_spam_wrath(
        self, cds: CooldownUsages = CooldownUsages(), delay=2
    ):
        def _rotation_callback():
            if (
    ...
    def _moonfire_insect_swarm_wrath(
        self, cds: CooldownUsages = CooldownUsages(), delay=2
    ):
        def _rotation_callback():
            if (
    ...
    def _moonfire_insect_swarm_starfire(
        self, cds: CooldownUsages = CooldownUsages(), delay=2
    ):
        def _rotation_callback():
            if (
    ...
    def moonfire_insect_swarm_wrath(
        self, cds: CooldownUsages = CooldownUsages(), delay=2
    ):
        return partial(self._set_rotation, name="moonfire_insect_swarm_wrath")(
            cds=cds, delay=delay
        )

    @simrotation("Moonfire -> Insect Swarm -> Starfire")
    def moonfire_insect_swarm_starfire(
        self, cds: CooldownUsages = CooldownUsages(), delay=2
    ):
        return partial(
            self._set_rotation, name="moonfire_insect_swarm_starfire"
        )(cds=cds, delay=delay)
    ...
    def spam_starfire(self, cds: CooldownUsages = CooldownUsages(), delay=2):
        return partial(self._set_rotation, name="spam_starfire")(
            cds=cds, delay=delay
        )

    @simrotation("Insect Swarm -> Wrath (Spam)")
    def insect_swarm_spam_wrath(
        self, cds: CooldownUsages = CooldownUsages(), delay=2
    ):
        return partial(self._set_rotation, name="insect_swarm_spam_wrath")(
            cds=cds, delay=delay
        )

    @simrotation("Wrath (Spam)")
    def spam_wrath(self, cds: CooldownUsages = CooldownUsages(), delay=2):
        return partial(self._set_rotation, name="spam_wrath")(
            cds=cds, delay=delay
        )

    @simrotation("Moonfire -> Starfire")
    def moonfire_starfire(
        self, cds: CooldownUsages = CooldownUsages(), delay=2
    ):
        return partial(self._set_rotation, name="moonfire_starfire")(
            cds=cds, delay=delay
        )

    @simrotation("Insect Swarm -> Starfire")
    def insect_swarm_starfire(
        self, cds: CooldownUsages = CooldownUsages(), delay=2
    ):
        return partial(self._set_rotation, name="insect_swarm_starfire")(
            cds=cds, delay=delay
        )

    @simrotation("Insect Swarm -> Wrath")
    def insect_swarm_wrath(
        self, cds: CooldownUsages = CooldownUsages(), delay=2
    ):
        return partial(self._set_rotation, name="insect_swarm_wrath")(
            cds=cds, delay=delay
        )

    @simrotation("Moonfire -> Wrath")
    def moonfire_wrath(self, cds: CooldownUsages = CooldownUsages(), delay=2):
        return partial(self._set_rotation, name="moonfire_wrath")(
            cds=cds, delay=delay
        )
