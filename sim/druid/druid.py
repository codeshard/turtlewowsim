...
class Druid(Character):
    ...
    def _nature_dot(
        ...
        if not hit:
            ...
            if spell == Spell.INSECT_SWARM:
                self.env.debuffs.add_insect_swarm_dot(
                    self, round(casting_time + cooldown, 2)
                )
    ...
    def insect_swarm_wrath_subrotation(
        ...
        if (
            not self.env.debuffs.is_insect_swarm_active(self)
    ...
    def insect_swarm_moonfire_wrath_subrotation(
        ...
        if (
            ...
            not self.env.debuffs.is_moonfire_active(self)
            and self.env.remaining_time() > 20
        ):
